      ...
      return(
        Object.keys(data).map((key, i) => (
          <div className="gx-form-group" key={i}>
            <label className="ant-col-md-12" name="Key" style={{width: "15%"}}>
              {apiKeyFormat(key)}
            </label>
            <label name="divider" style={{width: "8px"}}> : </label>
            <label name="Value" style={{width: "84%"}}>{ data[key] ? data[key] : '-' }</label>
          </div>
        ))
      )
    } else{
...
export default WidgetTableData;
